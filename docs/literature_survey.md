...
Survey này tổng hợp 11 công trình nghiên cứu liên quan trực tiếp đến dự án LucBinh, bao gồm 3 mảng chính:
...
Mục tiêu: Xác định research gap và định vị đóng góp của LucBinh so với state-of-the-art.
...
- **Ý nghĩa cho LucBinh:** Tiền đề cho Dempster-Shafer fusion trong multimodal ERC.
...
- **Khác biệt với LucBinh:** FedDISC giải quyết *"client thiếu modality"*, LucBinh giải quyết *"client có chất lượng data khác nhau"*. Approach hoàn toàn khác: diffusion recovery vs uncertainty-weighted aggregation.
...
- **Khác biệt với LucBinh:** FedMultiEmo là **utterance-level** (không model conversation context), domain khác (automotive vs dialogue). Không phải competitor trực tiếp.
...
- **Khác biệt với LucBinh:** FedEmoNet là **utterance-level SER** (không có dialogue context). Domain khác: isolated speech vs multi-turn conversation.
...
| **LucBinh EDL** | RoBERTa-B | **125M** | 63.09 | 56.33† | **Centralized** |
| **LucBinh EAFA** | RoBERTa-B | **125M** | 63.44 | 58.46† | **Federated** |

> *DialogueRNN 62.57% trên 4-class (happy+excited merged)  
> †LucBinh IEMOCAP chạy 6-class (khó hơn, không so sánh trực tiếp)
...
| **LucBinh EDL** | **88.09** | **Centralized** |
| **LucBinh EAFA** | **88.68** | **Federated** |
...
**Đóng góp LucBinh:** EAFA sử dụng EDL uncertainty làm tín hiệu chất lượng → tự động downweight clients kém → robust hơn FedAvg/FedProx.
...
**Đóng góp LucBinh:** EDL Head trên DialogueRNN → Dirichlet prediction + uncertainty estimation cho từng utterance trong conversation.
...
**Đóng góp LucBinh:** ECR — gradient auto-vanishes khi uncertain, không cần hard threshold.
...
**Đóng góp LucBinh:** Ablation study đầy đủ: CE vs EDL × FedAvg vs EAFA trên 3 datasets × 3 seeds.

---

## 5. Vị trí đóng góp của LucBinh (CẬP NHẬT)
...
| Aspect | FedDISC (NeurIPS 2025) | LucBinh |
|:-------|:----------------------|:--------|
...
- LucBinh: RoBERTa-**Base** 125M params (nhỏ hơn **2.8x**)
